...
function ConversationToolbar({ conversation, className, ...rest }) {
  ...
  return (
    <Toolbar {...rest} className={clsx(classes.root, className)}>
      <Tooltip title="Back">
        <RouterLink href="/chat" passHref>
          <IconButton className={classes.backButton} edge="start">
      ...
      <div className={classes.user}>
        ...
        <div className={classes.activity}>
          {conversation.otherUser.active ? (
            ...
            <Typography variant="body2">Active {moment(conversation.otherUser.lastActivity).fromNow()}</Typography>
          )}
        </div>
      </div>
      <Hidden smDown>
