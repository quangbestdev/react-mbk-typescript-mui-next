...
function ConversationListItem({ active, conversation, className, ...rest }) {
  const classes = useStyles()
  const lastMessage = conversation.messages[conversation.messages.length - 1]

  return (
    <RouterLink href="/chat/[id]" as={`/chat/${conversation.id}`} passHref>
      <ListItem
        {...rest}
        button
        className={clsx(
      ...
      >
        <ListItemAvatar>
          <Avatar alt="Person" className={classes.avatar} src={conversation.otherUser.avatar} />
        </ListItemAvatar>
        <ListItemText
        ...
        <div className={classes.details}>
          ...
          {conversation.unread > 0 && (
            <Label className={classes.unread} color={colors.red[500]} shape="rounded">
              {conversation.unread}
            </Label>
          )}
        </div>
